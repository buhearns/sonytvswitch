...
/**
 * A service class that provides a channel switch functionality for Sony TVs within TV-Browser for
 * Android.
 *
 * @author andan
 */
...
class TVBrowserSonyIPControlPlugin : Service() {
  ...
  @Inject lateinit var sonyControlRepository: SonyControlRepository
  ...
  private val getBinder: Plugin.Stub =
      object : Plugin.Stub() {
        ...
        override fun getMarkIcon(): ByteArray {
          val icon = BitmapFactory.decodeResource(resources, R.drawable.ic_action_share)
        ...
        override fun onProgramContextMenuSelected(
          ...
          if (pluginMenu.id == SWITCH_TO_CHANNEL) {
            Timber.d(
                " onProgramContextMenuSelected:switch to channel: $program.channel.channelName")
            try {
        ...
        override fun getContextMenuActionsForProgram(program: Program): Array<PluginMenu> {
          ...
          menuList.add(PluginMenu(SWITCH_TO_CHANNEL, title))
        ...
        override fun getMarkedPrograms(): LongArray {
          ...
          val values: Iterator<String> = mMarkingProgramIds.iterator()
        ...
        override fun handleFirstKnownProgramId(programId: Long) {
          if (programId == -1L) {
            ...
            val knownIds = mMarkingProgramIds!!.toTypedArray()
        ...
        override fun isMarked(programId: Long): Boolean {
          return programId != mRemovingProgramId &&
              mMarkingProgramIds!!.contains(programId.toString())
        }
