...
interface SonyService {
  ...
  @POST fun refreshToken(@Url url: String, @Body rpcRequest: JsonRpcRequest): Call<JsonRpcResponse>
}

class AddTokenInterceptor @Inject constructor(private val sessionManager: SessionManager) :
    Interceptor {
  ...
  override fun intercept(chain: Interceptor.Chain): okhttp3.Response {
    ...
    if (sessionManager.preSharedKey.isEmpty()) {
      ...
      if (sessionManager.challenge.isNotEmpty()) {
        builder.addHeader("Authorization", Credentials.basic("", sessionManager.challenge))
      }
      // execute request
      var newRequest = builder.build()
      val response = chain.proceed(newRequest)
      return if (response.code == HTTP_FORBIDDEN) {
        // It seems that the Sony TV responses with 403 (FORBIDDEN) code instead of 401
        // (UNAUTHORIZED) if token has expired or is invalid. Thus this case will not be handled
        // by the TokenAuthenticator class.
...
constructor(
  ...
  override fun authenticate(route: Route?, response: okhttp3.Response): Request? {
    ...
    return if (sessionManager.challenge.isBlank() &&
        request.url.toString().endsWith(SonyServiceUtil.SONY_ACCESS_CONTROL_ENDPOINT)) {
      null
    } else {
      val updatedToken = getAndStoreNewToken(sessionManager)
      request.newBuilder().addHeader("Cookie", updatedToken).build()
    }
  }
}

private fun getAndStoreNewToken(sessionManager: SessionManager): String {
  Timber.d("getNewToken: calling registration")
  var newToken = ""
  val response =
      sessionManager.sonyServiceProvider
          .get()
          .refreshToken(
  ...
  if (response.code() == HTTP_OK) {
    if (!response.headers()["Set-Cookie"].isNullOrEmpty()) {
      ...
      if (matcher.find()) {
        ...
        runBlocking { sessionManager.saveToken(newToken) }
      }
    }
  }
  return newToken
...
sealed class Result<T>(
  ...
  class Success<T>(code: Int, data: T) : Result<T>(data, code, status = Status.SUCCESS)

  // class Loading<T>(data: T? = null) : Resource<T>(data, status = Status.LOADING)
  class Error<T>(message: String, code: Int, data: T? = null) :
      Result<T>(data, code, message, Status.ERROR)

  class Loading<T> : Result<T>(status = Status.LOADING)
...
object SonyServiceUtil {
  ...
  inline fun <reified T> apiCall(call: () -> Response<JsonRpcResponse>): Result<T> {
    return try {
      ...
      if (response.isSuccessful) {
        ...
        when {
          jsonRpcResponse?.error != null -> {
            Result.Error(jsonRpcResponse.error.asJsonArray.get(1).asString, response.code())
          }
          else -> {
            Result.Success(
                response.code(),
      ...
      return Result.Error(e.message ?: "Unknown apiCall exception", 0)
    }
  }
}
