...
interface ControlDao {
  ...
  @Query("SELECT COUNT(*) FROM $CONTROL_TABLE") suspend fun getNumberOfControls(): Int

  @Query("SELECT * FROM $CONTROL_TABLE") fun getControls(): Flow<List<ControlEntity>>
  ...
  @Transaction
  suspend fun insertActiveControlWithChannels(
      controlEntity: ControlEntity,
      channels: List<ChannelEntity>,
      channelMaps: List<ChannelMapEntity>
  ) {
    insertControlWithChannels(controlEntity, channels, channelMaps)
    setActiveControl(controlEntity.uuid)
  }

  @Transaction
  @Query("SELECT * FROM $CONTROL_TABLE WHERE isActive = 1")
  fun getActiveControlWithChannelsFlow(): Flow<ControlEntityWithChannels>

  @Transaction
  @Query("SELECT * FROM $CONTROL_TABLE WHERE isActive = 1")
  suspend fun getActiveControlWithChannels(): ControlEntityWithChannels

  suspend fun setActiveControl(controlEntity: ControlEntity) {
    return setActiveControl(controlEntity.uuid)
  }

  @Insert suspend fun insertControls(vararg controls: ControlEntity)
  ...
  suspend fun insertFromSonyControls(sonyControls: SonyControls) {
    for ((index, sonyControl) in sonyControls.controls.withIndex()) {
      ...
      val channelMapList: MutableList<ChannelMapEntity> = mutableListOf()
      for (entry in sonyControl.channelMap) {
        val channelMapEntity =
            ChannelMapEntity(
                control_uuid = sonyControl.uuid, channelLabel = entry.key, uri = entry.value)
  ...
  @Delete suspend fun deleteControl(control: ControlEntity)

  @Query("DELETE FROM $CONTROL_TABLE WHERE uuid LIKE :uuid") suspend fun deleteControl(uuid: String)

  @Transaction
  suspend fun deleteControlAndSetActive(uuidDelete: String, uuidActive: String) {
    deleteControl(uuidDelete)
    setActiveControl(uuidActive)
  }

  @Update suspend fun update(control: ControlEntity)
  ...
  @Insert suspend fun insertChannels(channelList: List<ChannelEntity>)

  @Delete suspend fun deleteChannels(channelList: List<ChannelEntity>)
  ...
  suspend fun setChannelsForControl(
    ...
    insertChannels(
        channelList.filter { channelEntity -> channelEntity.control_uuid == controlEntity.uuid })
  }

  @Transaction
  suspend fun setChannelsForControl(channelList: List<ChannelEntity>, uuid: String) {
    deleteChannelsForControlUuid(uuid)
    insertChannels(channelList.filter { channelEntity -> channelEntity.control_uuid == uuid })
  }

  @Transaction
  suspend fun setChannelMapsForControl(channelMapList: List<ChannelMapEntity>, uuid: String) {
    deleteChannelMapsForControlUuid(uuid)
    insertChannelMaps(
        channelMapList.filter { channelMapEntity -> channelMapEntity.control_uuid == uuid })
  }
  ...
  @Insert suspend fun insertChannelMaps(channelMapList: List<ChannelMapEntity>)

  @Delete suspend fun deleteChannelMaps(channelMapList: List<ChannelMapEntity>)
